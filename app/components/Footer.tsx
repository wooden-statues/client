  ...
  return (
    <footer className="bg-dark-green px-[18%] py-[9.72vw] flex flex-col gap-10 relative z-10 max-xs:px-[10vw] max-xs:gap-6">
      ...
      <section className="flex w-full justify-between max-xs:flex-col max-xs:gap-8">
        <form
          className="flex flex-col gap-14 w-4/9 items-start max-xs:w-full max-xs:gap-8 max-xs:items-stretch"
          id="contact"
          onSubmit={onSubmit}
        >
          <input
          ...
          <input
            className="bg-light px-9 py-1.5 cursor-pointer rounded-lg text-[1.25vw] font-inter text-[#0C1203] max-xs:text-[2.5vw] disabled:opacity-60"
            type="submit"
            value={status === "loading" ? "Изпращане..." : "Изпрати"}
