    ...
    return (
        <nav className="flex justify-between items-center py-9 px-[15vw] bg-dark-green max-md:px-[7vw] max-s:py-6">
            <Link href="/" aria-label="Wooden Statues – Начало">
                <Image src="/logo.svg" alt="logo" width={116} height={41} className="w-[8.06vw] min-w-[80px] max-xs:min-w-[60px]" />
            </Link>

            {navLinks("flex gap-15 text-[1.39vw] text-[#F4ECE2] font-[575] max-md:text-[2vw] max-xs:hidden")}

            <svg onClick={() => setIsOpened(true)} className="hidden cursor-pointer max-xs:block" width="33" height="8" viewBox="0 0 33 8" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="Отвори меню">
                <rect width="33" height="2" fill="white" />
                <rect x="15" y="6" width="18" height="2" fill="white" />
            </svg>

            <section id="nav-overlay" className={`fixed bg-white w-full h-screen z-50 top-0 left-0 flex flex-col items-center justify-center transition duration-300 ease-linear ${isOpened ? "opacity-100 visible" : "opacity-0 invisible"}`} onClick={hideNavOverlay} aria-hidden={!isOpened}>
                <button className="bg-transparent cursor-pointer absolute top-5 right-5 border-none" onClick={() => setIsOpened(false)} aria-label="Затвори менюто">
