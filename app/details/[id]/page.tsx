            ...
            <main>
                <section className="grid grid-cols-2 w-4/5 justify-center gap-[5vw] pt-[8.5vw] pb-[5.83vw] relative mx-auto max-xs:grid-cols-1 max-xs:w-full max-xs:pt-0 max-xs:pb-[30vw] max-xs:gap-[7vw]">
                    ...
                    <section className='flex flex-col items-center gap-[1.7vw]'>
                        <article className='flex items-center justify-between w-9/10 max-xs:w-full'>
                            ...
                            <div className='flex items-center justify-center w-full relative'>
                                <Image ref={imageRef} src={product.images[activeImageIndex]} alt={product.title} width={1000} height={0} className="w-[70%] rounded-[10px] object-cover aspect-[2/3] max-xs:w-full max-xs:aspect-[4/5] max-xs:max-h-[120vw] max-xs:rounded-[0]" />

                                <p className='hidden absolute bg-[#656565B2] font-inter rounded-full text-white text-[2vw] font-light py-[0.5vw] px-[2vw] bottom-[3.5vw] right-[2vw] max-xs:block'>{activeImageIndex + 1}/{product.images.length}</p>
                            </div>
                    ...
                    <section className='flex flex-col items-start gap-[2vw] max-xs:items-center max-xs:gap-[6vw]'>
                        <h1 className='font-iowan font-bold text-[4.58vw] drop-shadow-[0_8px_4px_rgba(0,0,0,0.25)] max-xs:text-[7vw]/[7vw]'>{product.title}</h1>
                        <p className='font-inter text-[1.53vw] max-w-[35ch] max-xs:max-w-7/9 max-xs:text-[3vw]/[4vw] max-xs:text-center'>{product.description}</p>
                        <Link onClick={() => localStorage.setItem("item", product.title)} href="/requests" className='bg-brown text-white rounded-lg py-[0.8vw] px-[4vw] text-[1.3vw] max-xs:text-[2.6vw] max-xs:w-8/9 max-xs:text-center max-xs:py-[2vw] z-10'>Направи заявка</Link>
                    </section>
                </section>
